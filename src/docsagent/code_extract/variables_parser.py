...
class VariablesParser:
    ...
    def _get_default_code_paths(self) -> List[str]:
        ...
        starrocks_dir = project_root / "starrocks"
    ...
    def build_meta(self) -> Dict[str, Any]:
        """Build meta information for all FE config items"""
        
        logger.info("Building FE config meta information...")
        
        sources_files = self._get_source_code_paths()
        config_items = self.extract_all_configs(sources_files)
        
        # Group by config name (in case of duplicates)
        config_dict = {}
        for item in config_items:
            name = item.get("name")
            if name:
                if name in config_dict:
                    logger.warning(f"Duplicate config found: {name}")
