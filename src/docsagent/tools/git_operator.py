...
class GitOperator:
    ...
    def copy_and_commit(
        ...
        try:
            ...
            # Generate commit message with signoff
            commit_message = self._generate_commit_message(domain, changed_files, languages)
            commit_message = self._add_signoff(commit_message)
...
"""
    ...
    def _add_signoff(self, commit_message: str) -> str:
        """
        ...
        Returns:
            Commit message with Signed-off-by line
        """
        try:
            # Get user name and email from git config
            reader = self.repo.config_reader()
            user_name = reader.get_value('user', 'name', default='Unknown')
            user_email = reader.get_value('user', 'email', default='unknown@example.com')
            
            # Add signoff line
            signoff_line = f"\nSigned-off-by: {user_name} <{user_email}>"
            
            # Avoid duplicate signoff
            if signoff_line.strip() not in commit_message:
                commit_message += signoff_line
            
            return commit_message
            
        except Exception as e:
            logger.warning(f"Failed to add signoff: {e}")
            return commit_message
